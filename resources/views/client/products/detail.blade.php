        ...
        <div class="row px-xl-5">
                    ...
                    <div class="carousel-inner border">
                        @foreach ($productDetail as $value)
                            @if ($value->product_id == $product->id)
                                <input type="hidden" name="" value="{{ $value->product_id }}">
                                <div class="carousel-item active">
                                    <img class="w-100 h-100" src="{{ asset($value->image_path) }}" alt="Image">
                                </div>
                            @endif
                        @endforeach
                    </div>
            ...
            <div class="col-lg-7 pb-5">
                ...
                @if ($product->sale != null)
                    <h3 class="font-weight-semi-bold mb-4">${{ $product->price_new }}</h3>
                @else
                ...
                <div class="d-flex mb-3">
                    ...
                    @foreach ($sizes as $value)
                        <div class="custom-control custom-radio custom-control-inline">
                            <input type="radio" class="custom-control-input" id="size-{{ $value->size_id }}"
                                name="size_id" value="{{ $value->size_id }}">
                            <label class="custom-control-label"
                                for="size-{{ $value->size_id }}">{{ $value->size->name }}</label>
                        </div>
                    @endforeach
                    @if ($errors->has('size_id'))
                        <div class="text-danger">{{ $errors->first('size_id') }}</div>
                    @endif
                </div>
                ...
                <div class="d-flex mb-3">
                    ...
                    @foreach ($colors as $value)
                        <div class="custom-control custom-radio custom-control-inline">
                            <input type="radio" class="custom-control-input" id="color-{{ $value->color_id }}"
                                name="color_id" value="{{ $value->color_id }}">
                            <label class="custom-control-label"
                                for="color-{{ $value->color_id }}">{{ $value->color->name }}</label>
                        </div>
                    @endforeach
